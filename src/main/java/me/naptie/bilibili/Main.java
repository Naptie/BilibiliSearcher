...
public class Main {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		if (args.length >= 3) {
			...
			while (maleUsers.size() + femaleUsers.size() + unknownUsers.size() < max) {
				counter++;
				if (counter > 50) break;
				...
				for (int i = 0; i < videos.size(); i++) {
					if (maleUsers.size() + femaleUsers.size() + unknownUsers.size() == max) break;
			...
			List<List<Cell>> bodyForDisplay = new ArrayList<List<Cell>>() {{
				for (User user : users) {
					add(new ArrayList<Cell>() {{
						...
						add(new Cell("https://www.bilibili.com/video/" + user.getBvid()));
...
//		HttpGet httpGet = new HttpGet(url);
		...
		request.addRequestProperty("User-Agent", userAgent);
